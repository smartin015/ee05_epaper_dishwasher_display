 ...
 *   - Gray16 sprite itself: 1872*1404 / 2 = 1.32 MB (allocated by initGrayMode)
 *   - Source RGB888 decode: width*height*3 bytes
 *   - Quantization index:   width*height bytes (packed in-place into 4bpp halfway through)
 *
 * Peak working set ~= source_w*source_h*4 bytes. With 8 MB PSRAM and the sprite, your
 * source image must be smaller than about 1500x1100 pixels. The recommended workflow is
 * to pre-resize / dither the image in online_img2bitmap_.html, then export a 4bpp BMP at
 * panel resolution -- BMP decoding only allocates the final RGB888 once (no intermediate
 * resize buffer) and fits comfortably.
 *
 * Hardware: reTerminal E1003 (XIAO ESP32-S3 + 13.3" ED103TC2 16-level gray e-paper).
 *
 ...
 *           ITE ENABLE   21
 *           SD  CS       14
 *           SD  EN       39   (NOTE: differs from E1001/E1002/E1004 which use GPIO16)
 *           SD  DET      15
 *
...
static constexpr int    PIN_SD_MISO  = 8;   // already in Setup522 (TFT_MISO=8), but we re-init for symmetry
static constexpr int    PIN_SD_MOSI  = 9;   // shared with e-paper SPI bus
static constexpr int    PIN_SD_CS    = 14;
static constexpr int    PIN_SD_EN    = 39;   // E1003 only -- other reTerminal models use GPIO16
...
static bool show_image_on_panel(RgbImage* img) {
  ...
  LOG.printf(TAG " dithering Gray16 with %s, gamma=%.2f ...\n",
             kDitherNames[(int)DITHER_METHOD], DITHER_GAMMA);
  const uint32_t t0 = millis();
  if (!dither_image(img->pixels, W, H, PAL_GRAY16, DITHER_METHOD,
  ...
  LOG.println(TAG " update() -- panel refresh starts (~1-3s on Gray16)");
  LOG.flush();
...
void setup() {
  ...
  LOG.println(TAG " initGrayMode(16)");
  epaper.initGrayMode(GRAY_LEVEL16);
  epaper.fillSprite(TFT_GRAY_15);  // start from white
  log_mem("after initGrayMode");

  // Setup522 already declares TFT_MISO=8, so the shared SPI bus is correctly
  // initialized for SD on E1003. We still re-init explicitly for symmetry with
  // the E1001/E1002 examples (where Setup520/521 have TFT_MISO=-1 and MUST
  // re-init). This is a no-op functionally but makes the 5 examples uniform.
