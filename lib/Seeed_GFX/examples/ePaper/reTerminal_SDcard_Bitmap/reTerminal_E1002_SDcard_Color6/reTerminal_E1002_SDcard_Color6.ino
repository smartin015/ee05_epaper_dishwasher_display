 ...
 *               -> 4bpp packed -> pushImage into the 4bpp ED2208 sprite.
 *
 ...
 *           SD  EN       16
 *           SD  DET      15
 *           SD  MISO     8   (carrier-wired but NOT in Setup521; we add it at runtime)
 *
...
static constexpr int    PIN_SD_MISO  = 8;   // NOT in Setup521 (TFT_MISO=-1) -- we add it for SD
...
void setup() {
  ...
  // The ED2208 color-6 controller is write-only on this carrier, so Setup521
  // defines TFT_MISO=-1. epaper.begin() therefore initialized the shared SPI
  // bus WITHOUT routing MISO, but SD cards are full-duplex and SD.begin() needs
  // MISO to read card responses. Re-init the same SPI bus with MISO=GPIO8
  // added; the display's CS/RST/DC/BUSY are independent GPIOs and untouched.
