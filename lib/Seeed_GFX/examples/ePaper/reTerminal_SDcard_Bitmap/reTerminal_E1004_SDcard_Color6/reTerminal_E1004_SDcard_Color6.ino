 ...
 *   - 4bpp sprite:          1200*1600 / 2 = 960 KB (allocated by EPaper)
 *   - Source RGB888 decode: width*height*3 bytes  (panel size = 5.76 MB)
 *   - Quantization index:   width*height bytes    (panel size = 1.92 MB)
 *
 * Peak working set ~= source_w*source_h*4 bytes. At panel resolution Floyd-Steinberg
 * also needs ~11 MB of error buffer; dither_image() detects this and falls back to
 * DITHER_NONE. For best quality, pre-process the image with online_img2bitmap_.html
 * and write a 4bpp BMP at panel resolution -- BMP decoding has a much smaller peak.
 *
...
static constexpr int    PIN_SD_MISO  = 8;   // already in Setup523 (TFT_MISO=8), but we re-init for symmetry
...
//
// Default is BAYER8 here because color FS at panel size (1200x1600) needs
// more PSRAM than the 8 MB module typically has. For smaller source images
// you can safely switch to DITHER_FS / DITHER_JARVIS / DITHER_ATKINSON.
...
void setup() {
  ...
  // Setup523 already declares TFT_MISO=8, so the shared SPI bus is correctly
  // initialized for SD on E1004. We still re-init explicitly for symmetry with
